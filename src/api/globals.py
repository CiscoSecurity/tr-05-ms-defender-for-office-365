...
class GlobalsMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: ASGIApp) -> None:
        super().__init__(app, self.middleware_dispatch)

    @staticmethod
    async def middleware_dispatch(request: Request, call_next: Callable) -> Response:
        """Dispatch the request in a new context to allow globals to be used."""

        def _call_next() -> Awaitable[Response]:
            return call_next(request)

        return await copy_context().run(_call_next)
