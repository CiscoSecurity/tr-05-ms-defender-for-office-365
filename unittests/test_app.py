...
def calls():
    ...
    yield Call("GET", "/respond/observables", HTTPStatus.NOT_FOUND)
    yield Call("GET", "/respond/trigger", HTTPStatus.NOT_FOUND)
    yield Call("POST", "/watchdog", HTTPStatus.METHOD_NOT_ALLOWED)


@fixture(scope="module", params=calls(), ids=lambda call: f"{call.method} {call.route}")
def call(request):
    return request.param


def test_non_relay_call_failure(call, client):
    response = client.request(call.method, call.route)
